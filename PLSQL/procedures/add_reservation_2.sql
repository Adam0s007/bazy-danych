...
begin
    ...
    select 1 into tmp_person FROM PERSON p WHERE Add_Reservation_2.person_id = p.person_id;
    --sprawdzenie czy wycieczka o podanym id istnieje
    select 1 into tmp_trip FROM TRIP t WHERE Add_Reservation_2.trip_id = t.trip_id;
    ...
    select case
        when exists(select * FROM RESERVATION r
                             WHERE r.PERSON_ID = Add_Reservation_2.person_id
                            AND r.trip_id = Add_Reservation_2.trip_id) then 0
        else 1
        end
    into not_exist from dual;

    if not_exist = 0 then
        raise_application_error(-20003, 'Osoba z podaną nieodwołaną rezerwacją już istnieje');
    end if;
    --sprawdzamy czy wycieczka sie juz nie odbyla
	--sprawdzamy czy są wolne miejsca
    select case
            when exists(select * FROM AVAILABLE_TRIPS t
                                 WHERE t.trip_id = Add_Reservation_2.trip_id ) then 1
            else 0
            end
	...
	--dodajemy osobę
    INSERT INTO RESERVATION(trip_id,person_id,status) VALUES(Add_Reservation_2.trip_id,Add_Reservation_2.person_id,'N')
